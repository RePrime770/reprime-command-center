...
- SEC_EDGAR_USER_AGENT → Vercel env var — descriptive contact string the SEC requires on every EDGAR request, e.g. `RePrime Group g@reprime.com`. Read by `lib/sec-edgar/client.ts` (and reused by `lib/courtlistener/client.ts`). Optional: defaults to the RePrime contact if unset, but set it explicitly so the SEC can reach us. No account or signup needed — EDGAR is a free public API.
- COURTLISTENER_API_TOKEN → Vercel env var — Free Law Project API token for `lib/courtlistener/client.ts` (`/api/cron/courtlistener-poll`). Optional: anonymous requests work but are rate-limited; a free token raises the ceiling. Generate at https://www.courtlistener.com/profile/api/ → "API tokens". Sent as `Authorization: Token <token>`.
- CENSUS_API_KEY → Vercel env var — **required** for `lib/census/client.ts` (`/api/census/market`, `/api/cron/census-refresh`). Every Census Data API request needs a key; key-less requests 302-redirect to a "Missing Key" page. Free signup at https://api.census.gov/data/key_signup.html. Without it, the Census routes return 502 `census_missing_key`.
