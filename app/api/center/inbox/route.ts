  ...
  for (const panel of ['305', '718'] as const) {
    try {
      ...
      for (const c of waiting) {
        ...
        items.push({ channel: 'whatsapp', who: c.name || c.phone || 'Unknown', handle: `${c.phone || ''} · ${panel}`, preview, at: c.last_message_timestamp, link: c.phone ? `https://wa.me/${c.phone.replace(/\D/g, '')}` : '' })
      }
  ...
  try {
    ...
    for (const m of recent.slice(0, 40)) {
      try {
        ...
        items.push({ channel: 'email', who: fromH.name || fromH.address, handle: fromH.address, preview: (msg.headers['subject'] ? msg.headers['subject'] + ' — ' : '') + (msg.snippet || '').slice(0, 200), at: msg.receivedAt, link: `https://mail.google.com/mail/u/0/#inbox/${msg.threadId}` })
      } catch { /* skip */ }
    }
